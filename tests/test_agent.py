...
class TestGroupParser():
    def test_parse_text_group(self):
        # Return correctly
    ...
    def test_parse_text_group_real(self):
        # From Real Command
        ...
        assert GroupParser.parse_text_group("!set user2 Thai tea blizzard 2") == {
            "cmd": "set", "user_name": "user1", "item": "Thai tea blizzard", "num": "2"}
        assert GroupParser.parse_text_group("!set user2 Cookie&cream ice cream 1") == {
            "cmd": "set", "user_name": "user1", "item": "Cookie&cream ice cream", "num": "1"}
    ...
    def test_parse_text_group_thai_lang(self):
        # Testing Thai Language
        assert GroupParser.parse_text_group("!new เพลิน") == {
            "cmd": "new", "name": "เพลิน"}
        assert GroupParser.parse_text_group("!new เพลิน ร้านอาหารชื่อดังย่านวิภาวดี") == {
            "cmd": "new", "name": "เพลิน ร้านอาหารชื่อดังย่านวิภาวดี"}
        assert GroupParser.parse_text_group("!new ก๋วยจั๊บ3วัน") == {
            "cmd": "new", "name": "ก๋วยจั๊บ3วัน"}
        assert GroupParser.parse_text_group("!new โจ๊ก 3 ย่าน") == {
            "cmd": "new", "name": "โจ๊ก 3 ย่าน"}
        assert GroupParser.parse_text_group("!set แบ้ง อาหาร 3") == {
            "cmd": "set", "user_name": "แบ้ง", "item": "อาหาร", "num": "3"}
        assert GroupParser.parse_text_group("!set เอ็ม ผัดกะเพราหมูสับ ไม่ใส่กะเพรา 5") == {
            "cmd": "set", "user_name": "เอ็ม", "item": "ผัดกะเพราหมูสับ ไม่ใส่กะเพรา", "num": "5"}
...
def mock_agent(request):
    ...
    def agent_teardown():
        clean_test_db()
        print("\nCleaned mock agent")
    request.addfinalizer(agent_teardown)
    return mock_agent
...
class TestAgent():
    ...
    def test_agent_handle_text_message_fail(self, mock_agent):
        agent = mock_agent
        # Parser error
    ...
    def test_agent_handle_text_message_group_source(self, mock_agent):
        agent = mock_agent
    ...
    def test_agent_handle_text_message_room_source(self, mock_agent):
        agent = mock_agent
    ...
    def test_agent_handle_text_message_user_source(self, mock_agent):
        agent = mock_agent
    ...
    def test_agent_handle_new_order(self, mock_agent):
        agent = mock_agent
        assert agent._Agent__handle_new_order(
            room_id=TEST_ROOM_1, name=TEST_ORDER_1) != None

    def test_agent_handle_set_order(self, mock_agent):
        agent = mock_agent
    ...
    def test_agent_handle_del_order(self, mock_agent):
        agent = mock_agent
        agent._Agent__handle_new_order(
            room_id=TEST_ROOM_1, name=TEST_ORDER_1)
    ...
    def test_agent_handle_list_order(self, mock_agent):
        agent = mock_agent
    ...
    def test_agent_handle_close_order(self, mock_agent):
        agent = mock_agent
        agent._Agent__handle_new_order(room_id=TEST_ROOM_1, name=TEST_ORDER_1)
        assert agent._Agent__handle_close_order(room_id=TEST_ROOM_1) != None

    def test_agent_handle_open_order(self, mock_agent):
        agent = mock_agent
        agent._Agent__handle_new_order(room_id=TEST_ROOM_1, name=TEST_ORDER_1)
        agent._Agent__handle_close_order(room_id=TEST_ROOM_1)
        assert agent._Agent__handle_open_order(room_id=TEST_ROOM_1) != None

    def test_agent_handle_end_order(self, mock_agent):
        agent = mock_agent
        agent._Agent__handle_new_order(room_id=TEST_ROOM_1, name=TEST_ORDER_1)
        assert agent._Agent__handle_end_order(room_id=TEST_ROOM_1) != None

    def test_agent_handle_help(self, mock_agent):
        agent = mock_agent
